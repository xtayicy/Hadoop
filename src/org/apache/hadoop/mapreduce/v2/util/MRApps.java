...
public class MRApps extends Apps {
  ...
  private static void addToEnvironment$(Map<String,String> environment,String variable,String value,Configuration conf){
	  String val = environment.get(variable);
	  String separator = "Linux".equals(conf.get("mapred.remote.os"))?":":File.pathSeparator;
	  if(val == null){
		  val = value;
	  }else{
  ...
  private static void setMRFrameworkClasspath(
    ...
    if (conf.getBoolean(YarnConfiguration.IS_MINI_YARN_CLUSTER, false)) {
      ...
      addToEnvironment$(environment,Environment.CLASSPATH.name(),System.getProperty("java.class.path"),conf);
    }
    ...
    if (frameworkName == null) {
      ...
      for (String c : conf.getStrings(YarnConfiguration.YARN_APPLICATION_CLASSPATH,
        ...
        /*MRApps.addToEnvironment(environment, Environment.CLASSPATH.name(),
          c.trim(), conf);*/
    	  addToEnvironment$(environment, Environment.CLASSPATH.name(), c.trim(), conf);
      }
    }
  ...
  public static void setClasspath(Map<String, String> environment,
    ...
    /*MRApps.addToEnvironment(environment,
      classpathEnvVar, crossPlatformifyMREnv(conf, Environment.PWD), conf);*/
        addToEnvironment$(environment, classpathEnvVar, "Linux".equals(conf.get("mapred.remote.os")) ? "$PWD" : Environment.PWD.$(), conf);
    ...
    addToEnvironment$(environment, classpathEnvVar, MRJobConfig.JOB_JAR + Path.SEPARATOR + MRJobConfig.JOB_JAR, conf);
    /*MRApps.addToEnvironment(
        environment,
        classpathEnvVar,
        MRJobConfig.JOB_JAR + Path.SEPARATOR + "classes" + Path.SEPARATOR, conf);*/
    addToEnvironment$(environment, classpathEnvVar, MRJobConfig.JOB_JAR + Path.SEPARATOR + "classes" + Path.SEPARATOR, conf);
    /*MRApps.addToEnvironment(
        environment,
        classpathEnvVar,
        MRJobConfig.JOB_JAR + Path.SEPARATOR + "lib" + Path.SEPARATOR + "*", conf);*/
    addToEnvironment$(environment, classpathEnvVar, MRJobConfig.JOB_JAR + Path.SEPARATOR + "lib" + Path.SEPARATOR + "*", conf);
    /*MRApps.addToEnvironment(
        environment,
        classpathEnvVar,
        crossPlatformifyMREnv(conf, Environment.PWD) + Path.SEPARATOR + "*", conf);*/
    addToEnvironment$(environment, classpathEnvVar, "Linux".equals(conf.get("mapred.remote.os")) ? "$PWD" : Environment.PWD.$() + Path.SEPARATOR + "*", conf);
      ...
      for (Path p : paths) {
        ...
        if(!name.toLowerCase().endsWith(".jar")) {
          /*MRApps.addToEnvironment(
        	...
        	addToEnvironment$(environment, classpathEnvVar, "Linux".equals(conf.get("mapred.remote.os")) ? "$PWD" : Environment.PWD.$() + Path.SEPARATOR + name, conf);
        }
      }
    }
  }
